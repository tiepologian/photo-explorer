...
function createPhoto(lat, lng) {
    ...
    getFlickPhoto(lat, lng, 0, function (url, link) {
        ...
        $('#notes-ul').append('<li class="note" id=' + imgName + ' data-noteid=2 data-xpos=800 data-ypos=700><a href=' + link + ' target=_blank style="background-image:url(' + url + ')"></a></li>');
        $("#" + imgName).draggable();
        var numberW = Math.floor((Math.random() * (wW - 200)) + 1);
        var numberH = Math.floor((Math.random() * (wH - 200)) + 1);
        $("#" + imgName).animate({
            left: "+=" + numberW,
            top: "+=" + numberH
        }, 1000, function () {
...
function getFlickPhoto(lat, lng, d, cb) {
    ...
    $.ajax({
        ...
        link = "http://www.flickr.com/photos/" + result.photos.photo[depth].owner + "/" + result.photos.photo[depth].id;
        depth++;
